...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src")===$(`#${lightboxId} .lightboxImage`).attr("src")){activeImage=$(this)}});if(!activeImage){console.error("No active image found");return}
let imagesCollection=$("img.gallery-item").toArray();let index=imagesCollection.indexOf(activeImage[0]);let prevIndex=(index-1+imagesCollection.length)%imagesCollection.length;let prevImage=imagesCollection[prevIndex];$(`#${lightboxId} .lightboxImage`).attr("src",$(prevImage).attr("src"))},nextImage(lightboxId){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src")===$(`#${lightboxId} .lightboxImage`).attr("src")){activeImage=$(this)}});if(!activeImage){console.error("No active image found");return}
let imagesCollection=$("img.gallery-item").toArray();let index=imagesCollection.indexOf(activeImage[0]);let nextIndex=(index+1)%imagesCollection.length;let nextImage=imagesCollection[nextIndex];$(`#${lightboxId} .lightboxImage`).attr("src",$(nextImage).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
